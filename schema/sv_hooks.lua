...
function Schema:PlayerSwitchFlashlight(client, enabled)
	if (client:IsCombine()) then
		return true
    elseif ( client:GetCharacter():GetInventory():HasItem("flashlight") ) then
    	return true
    end
end
...
function Schema:PostPlayerLoadout(client)
	if (client:IsCombine()) then
		...
		elseif (client:Team() == FACTION_OTA and Schema:IsCombineRank(value, "EOW")) then
		    ...
		    client:SetArmor(200)
		    elseif (client:Team() == FACTION_OTA and !Schema:IsCombineRank(value, "EOW") and !Schema:IsCombineRank(value, "OWS")) then
		        client:SetMaxHealth(150)
            	client:SetHealth(150)
            	client:SetArmor(150)
