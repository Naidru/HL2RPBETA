		...
		if (IsValid(ent) and bDraw) then
			...
			if (IsValid(camera) and camera:GetClass() == "npc_combine_camera") then
				...
				if (bulbColor.g == 128) then
					statusText = "Status Orange"
				elseif (bulbColor.g == 0) then
					statusText = "Status Red"
				end
				...
				render.PushRenderTarget(ent.tex)
					...
					cam.Start2D()
						draw.SimpleText("<:: C-i" .. camera:EntIndex() .. " ::>", "Frak-Regular", 4, 6)
						draw.SimpleText("<:: " .. statusText .. " ::>", "Frak-Regular", 4, 6 + draw.GetFontHeight("Frak-Regular"), bulbColor)
						draw.SimpleText(signalText, "Frak-Regular", 4, 252 - draw.GetFontHeight("Frak-Regular"))
...
function PLUGIN:HUDPaint()
	...
	if (client:IsCombine()) then
		...
		for k, v in ipairs(self.hudObjectives) do
			...
			draw.SimpleText(v, "Frak-Regular", info.x, info.y, textColor, TEXT_ALIGN_RIGHT)
			...
			elseif (!(!data.isLost and unit:GetMoveType() == MOVETYPE_NOCLIP)) then
				...
				if (toScreen.visible) then
					...
					if (data.isLost) then
						...
						draw.SimpleText(text, "Frak-Regular", toScreen.x, toScreen.y, color, 1, 1)
						toScreen.y = toScreen.y + fontHeight
						draw.SimpleText(text2, "Frak-Regular", toScreen.x, toScreen.y, colorRed, 1, 1)
						toScreen.y = toScreen.y + fontHeight
						draw.SimpleText("<:: Removing " .. timeUntil .. "s ::>", "Frak-Regular", toScreen.x, toScreen.y, colorRed, 1, 1)
					else
						local text2 = "<:: Received " .. timeSince .. "s ::>"
						draw.SimpleText(text, "Frak-Regular", toScreen.x, toScreen.y, color, 1, 1)
						toScreen.y = toScreen.y + fontHeight
						draw.SimpleText(showDetail and text2 or lowDetailText, "Frak-Regular", toScreen.x, toScreen.y, color_white, 1, 1)

						if (data.isKnockedOut) then
							toScreen.y = toScreen.y + fontHeight
							draw.SimpleText("<:: Unconscious ::>", "Frak-Regular", toScreen.x, toScreen.y, colorRed, 1, 1)
						end
					end
				end
			end
		...
		for i, data in ipairs(self.requestLocations) do
			if (curTime - data.time >= requestExpiry) then
				...
				if (toScreen.visible) then
					...
					draw.SimpleText("<:: Assistance Request ::>", "Frak-Regular", toScreen.x, toScreen.y, requestColor, 1, 1)
					toScreen.y = toScreen.y + fontHeight
					draw.SimpleText(showDetail and text2 or lowDetailText, "Frak-Regular", toScreen.x, toScreen.y, color_white, 1, 1)
					toScreen.y = toScreen.y + fontHeight
					draw.SimpleText("<:: Removing " .. timeUntil .. "s ::>", "Frak-Regular", toScreen.x, toScreen.y, colorRed, 1, 1)
		...
		for combineCamera, data in pairs(self.cameraData) do
			if (IsValid(combineCamera)) then
				...
				if (toScreen.visible) then
					...
					draw.SimpleText(showDetail and text1 or lowDetailText, "Frak-Regular", toScreen.x, toScreen.y, colorObject, 1, 1)
					...
					if (type(data) == "table") then
						...
						if (#violations > 0) then
							toScreen.y = toScreen.y + fontHeight
							draw.SimpleText("<:: Violations Within Sights ::>", "Frak-Regular", toScreen.x, toScreen.y, colorRed, 1, 1)

							for i, violation in ipairs(violations) do
								toScreen.y = toScreen.y + fontHeight
								draw.SimpleText(showDetail and violation or lowDetailText, "Frak-Regular", toScreen.x, toScreen.y, color_white, 1, 1)
							end
						end
					else
						toScreen.y = toScreen.y + fontHeight
						draw.SimpleText("<:: Disabled ::>", "Frak-Regular", toScreen.x, toScreen.y, colorRed, 1, 1)
					end
				end
			end
		end
			...
			if (v:GetCharacter() and (!v:IsCombine() or v:GetNetVar("IsBiosignalGone", false)) and beholderEyePos:Distance(v:GetPos()) <= maximumDistance and v:GetMoveType() != MOVETYPE_NOCLIP) then
				...
				if (toScreen.visible and beholder:IsLineOfSightClear(v)) then
					...
					if (ix.config.Get("useTagSystem") and beholderEyePos:Distance(v:GetPos()) <= (maximumDistance / 6) and !v:GetCharacter():GetData("IsCIDTagGone") and CID != "") then
						...
						draw.SimpleText(showDetail and text or lowDetailText, "Frak-Regular", toScreen.x, toScreen.y, color, 1, 1)
						toScreen.y = toScreen.y + fontHeight
					end
					...
					if (#violations > 0) then
						draw.SimpleText("<:: Possible Violation ::>", "Frak-Regular", toScreen.x, toScreen.y, colorRed, 1, 1)
